...
import {
  ...
  finalizeWorkOrderBlockedMessage,
  finalizeWorkOrderSuccessMessage,
...
export function WorkerAssignmentsPanel({
  ...
  function updateAssignmentInSession(updated: WorkerAssignmentRecord) {
    setSession((current) =>
      current
        ? {
            ...current,
  ...
  async function handleFinalizeWorkOrder() {
    if (!selectedAssignment) {
      setErrorMessage("Select an assignment first.");
      return;
    }

    if (!canFinalizeWorkOrder(selectedAssignment)) {
      setErrorMessage(finalizeWorkOrderBlockedMessage());
      return;
    }
    ...
    if (isBrowserOffline()) {
      setErrorMessage("You are offline. Finalize was not submitted.");
      return;
    }
    ...
    try {
      const response = await fetch(`/api/worker/work-orders/${selectedAssignment.workOrderId}/finalize`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ pin })
      });
      ...
      if (selectedAssignment) {
        setFinalizeSummary(buildFinalizeSummary(selectedAssignment, payload));
      }
      ...
      setIsBusy(false);
      setErrorMessage("Network error. Finalize was not submitted.");
    }
  }
        ...
        <>
          ...
          {selectedAssignment && (showVinAndComplete || showSummaryOnly) ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40">
              ...
              <p className="mt-1 text-sm text-slate-600">
                Mark each service performed on this vehicle.
              </p>
              ...
              <div className="mt-4">
                <ServiceChecklistPanel
                  ...
                  disabled={isBusy || Boolean(showSummaryOnly) || !/^\d{6}$/u.test(pin)}
                  onAssignmentChange={updateAssignmentInSession}
                />
              </div>

              {canFinalizeWorkOrder(selectedAssignment) && !showSummaryOnly ? (
                <button
                  type="button"
                  onClick={() => void handleFinalizeWorkOrder()}
                  disabled={isBusy}
                  className="mt-4 w-full rounded-xl bg-slate-900 px-4 py-3.5 text-base font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
                >
                  {isBusy ? "Finalizing…" : "Finalize job"}
                </button>
              ) : !showSummaryOnly ? (
                <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm text-amber-800">
                  {finalizeWorkOrderBlockedMessage()}
