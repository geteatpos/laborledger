...
export function EmployeeJobWorkflowPanel({
  ...
  function updateAssignmentInContext(updated: WorkerAssignmentRecord) {
    setContext((current) =>
      current
        ? {
            ...current,
  ...
  async function handleFinalizeJob() {
    ...
    try {
      const response = await fetch(`/api/field/jobs/${selectedAssignment.workOrderId}/finalize`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({})
      });
    ...
    <div className="space-y-5">
      ...
      {!summaryOnly && step === "service" && selectedAssignment ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Mark each service performed on this vehicle. No prices are shown here.
          </p>
          ...
          <div className="mt-4">
            <ServiceChecklistPanel
              ...
              employeeName={context?.session.employeeName}
              disabled={isBusy}
              onAssignmentChange={updateAssignmentInContext}
            />
          </div>

          {canFinalizeWorkOrder(selectedAssignment) ? (
            <button
              type="button"
