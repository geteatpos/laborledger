export type WorkerAssignmentRecord = {
  ...
  serviceLine?: { id: string; serviceNameSnapshot: string } | null;
  lastConfirmation: {
    scanEventId: string;
    acceptedAt: string;
    enteredVin: string;
...
export function canFinalizeWorkOrder(assignment: WorkerAssignmentRecord) {
  if (assignment.status === "COMPLETED" || assignment.status === "INVOICED" || assignment.status === "CANCELLED") {
    return false;
  }

  return assignment.serviceLines.some((line) => Boolean(line.completion));
}

export function finalizeWorkOrderBlockedMessage() {
  return "Mark at least one service complete before finalizing this job.";
}

export function finalizeWorkOrderSuccessMessage() {
  return "Job finalized. Incomplete services remain visible as not performed.";
}
