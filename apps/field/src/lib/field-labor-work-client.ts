...
function companyId(session: FieldSessionData): string | null {
...
export function isFieldLaborWorkConfigured(session?: FieldSessionData): boolean {
...
async function callFieldLaborWork<T>(
  ...
  const resolvedCompanyId = companyId(session);
