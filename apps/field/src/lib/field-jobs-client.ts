...
export function isFieldJobsConfigured(session?: FieldSessionData): boolean {
...
export async function callWorkerMarkWorkOrderService(
  session: FieldSessionData,
  workOrderId: string,
...
export async function callWorkerUnmarkWorkOrderService(
  session: FieldSessionData,
  workOrderId: string,
  serviceCatalogItemId: string
): Promise<FieldWorkerApiResult> {
  ...
  const apiResponse = await fetch(
    `${API_BASE_URL}/worker/work-orders/${workOrderId}/services/unmark`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
...
export async function callWorkerFinalizeWorkOrder(
  session: FieldSessionData,
  workOrderId: string
): Promise<FieldWorkerApiResult> {
  const companyId = workerCompanyId(session);
...
export type FieldVinDecodePreview = {
  vin: string;
  year: number | null;
  make: string | null;
  model: string | null;
...
export async function callWorkerDecodeVin(
  session: FieldSessionData,
  vin: string
): Promise<FieldWorkerApiResult<FieldVinDecodePreview>> {
  const companyId = workerCompanyId(session);
  if (!companyId) {
    return {
      ok: false,
      status: 503,
      payload: { message: fieldJobsNotConfiguredMessage() } as unknown as FieldVinDecodePreview
    };
  }

  const apiResponse = await fetch(`${API_BASE_URL}/worker/jobs/decode-vin`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ companyId, pin: session.pin, vin }),
    cache: "no-store"
  });

  const payload = (await apiResponse.json().catch(() => ({}))) as FieldVinDecodePreview;
  return { ok: apiResponse.ok, status: apiResponse.status, payload };
}
