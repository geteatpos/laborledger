...
describe("labor pay billing", () => {
  ...
  it("creates, lists, reopens, and exports labor billing drafts", async () => {
    const fixture = await createFixture();
    const shiftId = await punchCompleteShift(fixture);
    ...
    const previewBeforeDraft = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/preview?weekStart=${WEEK_START}`
      )
      .set("Cookie", fixture.adminSession)
    ...
    expect(previewBeforeDraft.body.draftSupported).toBe(true);
    expect(previewBeforeDraft.body.totals.approvedShiftCount).toBe(1);

    const created = await request(httpServer)
      .post(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts?weekStart=${WEEK_START}`
      )
      .set("Cookie", fixture.adminSession)
      .expect(201);

    expect(created.body.id).toBeTruthy();
    expect(created.body.snapshot.totals.approvedShiftCount).toBe(1);
    expect(created.body.snapshot.shiftRows).toHaveLength(1);

    const list = await request(httpServer)
      .get(`/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts`)
      .set("Cookie", fixture.adminSession)
      .expect(200);
    ...
    const detail = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts/${created.body.id}`
      )
      .set("Cookie", fixture.adminSession)
      .expect(200);

    expect(detail.body.snapshot.totals.payableMinutes).toBe(
      previewBeforeDraft.body.totals.payableMinutes
    );

    const payrollCsv = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts/${created.body.id}/payroll.csv`
      )
      .set("Cookie", fixture.adminSession)
      .expect(200);

    expect(payrollCsv.headers["content-type"]).toContain("text/csv");
    expect(payrollCsv.text).toContain("employee_name");
    expect(payrollCsv.text).toContain("Labor Employee");

    const clientCsv = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts/${created.body.id}/client-billing.csv`
      )
      .set("Cookie", fixture.adminSession)
      .expect(200);

    expect(clientCsv.headers["content-type"]).toContain("text/csv");
    expect(clientCsv.text).toContain("service_client");
  });

  it("keeps draft CSV stable after underlying shift data changes", async () => {
    const fixture = await createFixture();
    const shiftId = await punchCompleteShift(fixture);
    ...
    const payrollBefore = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts/${created.body.id}/payroll.csv`
      )
      .set("Cookie", fixture.adminSession)
    ...
    await prisma.employeeRate.create({
      data: {
        ...
        employeeId: (await prisma.employee.findFirstOrThrow({ where: { companyId: fixture.companyId } }))
          .id,
        rateMinorUnits: 9900,
    ...
    const livePreview = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/preview?weekStart=${WEEK_START}`
      )
      .set("Cookie", fixture.adminSession)
    ...
    const payrollAfter = await request(httpServer)
      .get(
        `/company-operations/companies/${fixture.companyId}/labor-pay-billing/drafts/${created.body.id}/payroll.csv`
      )
      .set("Cookie", fixture.adminSession)
    ...
    expect(payrollAfter.text).toBe(payrollBefore.text);
    expect(livePreview.body.totals.employeeGrossEstimateMinor).not.toBe(
      created.body.snapshot.totals.employeeGrossEstimateMinor
    );
  });

  it("rejects draft access without authentication and across companies", async () => {
    const fixtureA = await createFixture("Alpha");
    const fixtureB = await createFixture("Beta");
    const shiftId = await punchCompleteShift(fixtureA);
    ...
    const created = await request(httpServer)
      .post(
        `/company-operations/companies/${fixtureA.companyId}/labor-pay-billing/drafts?weekStart=${WEEK_START}`
      )
      .set("Cookie", fixtureA.adminSession)
      .expect(201);
    ...
    await request(httpServer)
      .get(
        `/company-operations/companies/${fixtureB.companyId}/labor-pay-billing/drafts/${created.body.id}`
      )
      .set("Cookie", fixtureA.adminSession)
      .expect(403);
    ...
    const invoiceCount = await prisma.clientInvoice.count({
      where: { companyId: fixtureA.companyId, status: ClientInvoiceStatus.ISSUED }
    });
