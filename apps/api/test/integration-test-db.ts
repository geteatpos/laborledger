...
const FORBIDDEN_PRODUCTION_DB_NAMES = new Set(["servihour", "laborledger_prod", "production"]);

export function assertSafeIntegrationDatabaseUrl(databaseUrl: string) {
  let pathname = "";
  try {
    pathname = new URL(databaseUrl).pathname.replace(/^\//, "").split("?")[0] ?? "";
  } catch {
    throw new Error(`Invalid INTEGRATION_DATABASE_URL / DATABASE_URL for tests: ${databaseUrl}`);
  }

  const dbName = pathname.toLowerCase();
  if (!dbName || FORBIDDEN_PRODUCTION_DB_NAMES.has(dbName) || !/(test|ci|integration)/i.test(dbName)) {
    throw new Error(
      `Refusing to run integration tests against database "${pathname}". ` +
        `Set INTEGRATION_DATABASE_URL (or DATABASE_URL) to a dedicated test database whose name contains test/ci/integration ` +
        `(e.g. servihour_test).`
    );
  }
}
...
assertSafeIntegrationDatabaseUrl(INTEGRATION_DATABASE_URL);
