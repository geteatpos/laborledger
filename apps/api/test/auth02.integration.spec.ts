...
describe("AUTH02 password reset and user invitations", () => {
  ...
  it("supports supervisor invitations with authorization and listing", async () => {
    const fixture = await createCompanyFixture("SupervisorCo");

    const employeeEmail = `employee-${randomBytes(2).toString("hex")}@example.com`;
    const employeePassword = `Employee!${randomBytes(4).toString("hex")}`;
    ...
    const employeeSession = await login(employeeEmail, employeePassword);
    ...
    const otherCompany = await createCompanyFixture("OtherSupervisorCo");
    await request(httpServer)
      .post("/auth/invitations")
      .set("Cookie", fixture.adminSession)
      .send({
        companyId: otherCompany.companyId,
        email: "cross-supervisor@example.com",
        role: "SUPERVISOR"
      })
      .expect(403);
    ...
    const supervisorInvite = await request(httpServer)
      .post("/auth/invitations")
      .set("Cookie", fixture.adminSession)
      .send({
        companyId: fixture.companyId,
    ...
    expect(supervisorInvite.body.role).toBe("SUPERVISOR");
    expect(supervisorInvite.body.status).toBe("PENDING");
    expect(emailSendSpy).toHaveBeenCalled();

    const pendingSupervisors = await request(httpServer)
      .get(`/company-operations/companies/${fixture.companyId}/supervisors`)
      .set("Cookie", fixture.adminSession)
      .expect(200);

    expect(
      pendingSupervisors.body.some(
        (row: { email: string; status: string }) =>
          row.email === "new-supervisor@example.com" && row.status === "INVITED"
      )
    ).toBe(true);

    const adminInvite = await request(httpServer)
      .post("/auth/invitations")
      .set("Cookie", fixture.adminSession)
      .send({
        companyId: fixture.companyId,
    ...
    expect(adminInvite.body.role).toBe("COMPANY_ADMIN");

    const rawToken = randomBytes(32).toString("base64url");
    await prisma.invitation.update({
      where: { id: supervisorInvite.body.id },
      data: { tokenHash: createHash("sha256").update(rawToken).digest("hex") }
    });

    await request(httpServer)
      .post("/auth/invitations/accept")
      .send({ token: rawToken, password: "SupervisorPass!123", name: "New Supervisor" })
      .expect(200);

    const activeSupervisors = await request(httpServer)
      .get(`/company-operations/companies/${fixture.companyId}/supervisors`)
      .set("Cookie", fixture.adminSession)
      .expect(200);

    expect(
      activeSupervisors.body.some(
        (row: { email: string; status: string; userId: string | null }) =>
          row.email === "new-supervisor@example.com" &&
          row.status === "ACTIVE" &&
          row.userId !== null
      )
    ).toBe(true);
