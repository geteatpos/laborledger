...
describe("client-invoice-email-content", () => {
  ...
  it("includes due terms balance and payment instructions from invoice snapshot input", () => {
    const { textBody, htmlBody } = buildClientInvoiceEmailBodies({
      ...baseInput,
      dueDate: new Date("2026-07-07T12:00:00.000Z"),
      paymentTermsDays: 15,
      balanceMinor: 9900,
      paymentInstructions: "Pay by ACH to the account on file.",
      taxId: "TAX-123"
    });

    expect(textBody).toContain("Due date: Jul 7, 2026");
    expect(textBody).toContain("Payment terms: Net 15");
    expect(textBody).toContain("Balance: $99.00");
    expect(textBody).toContain("Pay by ACH to the account on file.");
    expect(textBody).toContain("Tax ID: TAX-123");
    expect(htmlBody).toContain("Payment terms: Net 15");
  });
