...
describe("labor pay billing helpers", () => {
  ...
  it("builds immutable draft snapshots from preview data", () => {
    const preview = {
      periodStart: "2026-04-06",
      periodEnd: "2026-04-12",
      weekStatus: "OPEN" as const,
      dataSource: "live_approved" as const,
      dataSourceLabel: "Not closed — live approved shifts only",
      snapshotVersion: null,
      currencyCode: "USD",
      employeePayPrep: aggregateEmployeePayRows(sampleRows, "2026-04-06", "2026-04-12"),
      clientLaborBilling: aggregateClientLaborBillingRows(sampleRows, "2026-04-06", "2026-04-12"),
      excludedShifts: [],
      totals: summarizeLaborRows(sampleRows),
      filters: {
    ...
    const snapshot = buildLaborBillingDraftSnapshot({
      ...
      preview,
      shiftRows: sampleRows
    });

    expect(snapshot.shiftRows).toHaveLength(2);
    expect(snapshot.totals.payableMinutes).toBe(480);
    expect(snapshot.generatedByName).toBe("Admin User");
  });
});
