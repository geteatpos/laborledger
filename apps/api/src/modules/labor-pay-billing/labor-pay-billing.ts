...
export function buildLaborBillingDraftSnapshot(input: {
  generatedAt: string;
  generatedByUserId: string;
  generatedByName: string | null;
  preview: Omit<LaborPayBillingPreview, "company" | "draftSupported">;
  shiftRows: ShiftLaborRow[];
}): LaborBillingDraftSnapshot {
  return {
    generatedAt: input.generatedAt,
    generatedByUserId: input.generatedByUserId,
    generatedByName: input.generatedByName,
    periodStart: input.preview.periodStart,
    periodEnd: input.preview.periodEnd,
    weekStatus: input.preview.weekStatus,
    dataSource: input.preview.dataSource,
    dataSourceLabel: input.preview.dataSourceLabel,
    snapshotVersion: input.preview.snapshotVersion,
    currencyCode: input.preview.currencyCode,
    filters: input.preview.filters,
    shiftRows: input.shiftRows,
    employeePayPrep: input.preview.employeePayPrep,
    clientLaborBilling: input.preview.clientLaborBilling,
    excludedShifts: input.preview.excludedShifts,
