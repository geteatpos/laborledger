...
export function buildClientInvoiceEmailBodies(input: BuildInvoiceEmailInput) {
  ...
  const dueLine = input.dueDate ? `Due date: ${formatDate(input.dueDate)}` : "Due date: Not set";
  const termsLine =
    typeof input.paymentTermsDays === "number" ? `Payment terms: Net ${input.paymentTermsDays}` : "Payment terms: Not set";
  const balanceMinor = typeof input.balanceMinor === "number" ? input.balanceMinor : input.totalMinor;
  const paymentInstructions = input.paymentInstructions?.trim() || "No payment instructions provided.";
  ...
  const textBody = [
    ...
    `Balance: ${formatMoney(balanceMinor, input.currencyCode)}`,
    dueLine,
    termsLine,
  ...
  const htmlBody = [
    ...
    `Total: ${escapeHtml(formatMoney(input.totalMinor, input.currencyCode))}<br/>`,
    `<strong>Balance: ${escapeHtml(formatMoney(balanceMinor, input.currencyCode))}</strong><br/>`,
    `${escapeHtml(dueLine)}<br/>`,
    `${escapeHtml(termsLine)}<br/>`,
    `Payment instructions: ${escapeHtml(paymentInstructions)}</p>`,
...
function formatInvoiceEmailContactLines(
  input: Pick<BuildInvoiceEmailInput, "contactPhone" | "contactBillingEmail" | "contactName" | "taxId">
) {
  const lines: string[] = [];

  if (input.taxId?.trim()) {
    lines.push(`Tax ID: ${input.taxId.trim()}`);
  }
