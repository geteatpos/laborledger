...
export class CompanyScopeService {
  ...
  async getCompanyAccessContext(
    ...
    if (companyAdminMembership) {
      // locationId null = owner/admin sees all branches; string cuid = single-branch scope.
      // Never treat falsy numeric 0 as a location — only string | null.
      const branchId =
        typeof companyAdminMembership.locationId === "string" &&
        companyAdminMembership.locationId.length > 0
          ? companyAdminMembership.locationId
          : null;
      return this.buildContext(company, "company_admin", branchId ? [branchId] : null);
    }
