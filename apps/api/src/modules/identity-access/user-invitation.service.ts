...
const ALLOWED_COMPANY_INVITATION_ROLES = {
  COMPANY_ADMIN: CompanyRole.COMPANY_ADMIN,
  SUPERVISOR: CompanyRole.SUPERVISOR
} as const;

type AllowedCompanyInvitationRole = keyof typeof ALLOWED_COMPANY_INVITATION_ROLES;

type CreateUserInvitationInput = {
  companyId: string;
  email: string;
  role: AllowedCompanyInvitationRole;
};
...
export class UserInvitationService {
  ...
  async createCompanyInvitation(principal: AuthenticatedPrincipal, input: CreateUserInvitationInput) {
    ...
    const companyRole = ALLOWED_COMPANY_INVITATION_ROLES[input.role];
    if (!companyRole) {
    ...
    const { textBody, htmlBody } = buildInvitationEmailBodies({
      ...
      roleLabel: companyRole === CompanyRole.SUPERVISOR ? "Supervisor" : "Company Administrator",
