...
export function calculateWorkedMinutes(events: PunchEventRecord[]) {
  ...
  let breakMinutes = 0;
  const breakStarts = events.filter((event) => event.action === PunchAction.BREAK_START);
  const breakEnds = events.filter((event) => event.action === PunchAction.BREAK_END);

  for (let i = 0; i < Math.min(breakStarts.length, breakEnds.length); i++) {
    breakMinutes +=
      breakEnds[i].breakMinutes ?? minutesBetween(breakStarts[i].eventUtc, breakEnds[i].eventUtc);
  }
...
export function calculateBreakDurationMinutes(events: PunchEventRecord[]) {
  ...
  let total = 0;
  for (let i = 0; i < Math.min(breakStarts.length, breakEnds.length); i++) {
    total += breakEnds[i].breakMinutes ?? minutesBetween(breakStarts[i].eventUtc, breakEnds[i].eventUtc);
  }
  return total;
}
