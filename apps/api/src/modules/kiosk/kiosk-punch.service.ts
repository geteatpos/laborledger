...
export class KioskPunchService {
  ...
  async lookup(kiosk: KioskContext, input: LookupRequest) {
    ...
    return this.lookupByEmployee(kiosk, employee);
  }

  /** Mobile Bearer sessions resolve the employee from the token — no PIN. */
  async lookupByEmployeeId(kiosk: KioskContext, employeeId: string) {
    const employee = await this.resolveEmployeeById(kiosk.companyId, employeeId);
    return this.lookupByEmployee(kiosk, employee);
  }
  ...
  async processPunch(kiosk: KioskContext, input: PunchRequest) {
    ...
    const employee = await this.resolveEmployeeByPin(kiosk.companyId, pin);
    return this.processPunchForEmployee(kiosk, employee, input);
  }

  /** Mobile Bearer sessions punch by employee id from the session — no PIN. */
  async processPunchByEmployeeId(
    kiosk: KioskContext,
    employeeId: string,
    input: Omit<PunchRequest, "pin">
  ) {
    const employee = await this.resolveEmployeeById(kiosk.companyId, employeeId);
    return this.processPunchForEmployee(kiosk, employee, input);
  }

  private async lookupByEmployee(
    kiosk: KioskContext,
    employee: { id: string; fullName: string; companyId: string }
  ) {
    const shift = await this.resolveShiftForLookup(kiosk, employee.id);
    return this.buildSessionResponse(shift, employee, []);
  }

  private async processPunchForEmployee(
    kiosk: KioskContext,
    employee: { id: string; fullName: string; companyId: string },
    input: Omit<PunchRequest, "pin">
  ) {
  ...
  private async resolveEmployeeById(companyId: string, employeeId: string) {
