...
import {
  ...
  APPROVALS_STATUS_EXCEPTIONS,
  APPROVALS_STATUS_NEEDS_CORRECTION,
  APPROVALS_STATUS_NEEDS_REVIEW
} from "../lib/time-module-copy";
import {
...
export function ReviewWorkspace({
  ...
  const statusCounts = useMemo(() => summarizeReviewShiftCounts(shifts), [shifts]);
  ...
  return (
    <>
      <div className="mb-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <StatusSummaryCard
          label={APPROVALS_STATUS_NEEDS_REVIEW}
          count={statusCounts.needsReview}
          href={buildReviewHref({ status: "needs_review" })}
          active={initialStatus === "needs_review"}
          tone="amber"
        />
        <StatusSummaryCard
          label={APPROVALS_STATUS_APPROVED}
          count={statusCounts.approved}
          href={buildReviewHref({ status: "approved" })}
          active={initialStatus === "approved"}
          tone="emerald"
        />
        <StatusSummaryCard
          label={APPROVALS_STATUS_NEEDS_CORRECTION}
          count={statusCounts.incomplete}
          href={buildReviewHref({ status: "incomplete" })}
          active={initialStatus === "incomplete"}
          tone="red"
        />
        <StatusSummaryCard
          label={APPROVALS_STATUS_EXCEPTIONS}
          count={statusCounts.exceptions}
          href={buildReviewHref({ status: "exceptions" })}
          active={initialStatus === "exceptions"}
          tone="slate"
        />
      </div>
      ...
      {filteredShifts.length === 0 ? (
        <EmptyState
          title="No shifts match this approval queue"
          description="Completed punch sessions for the selected week and filters will appear here for review."
          action={
            <Link
              href="/corrections"
...
function StatusSummaryCard({
  ...
  const toneClasses = {
    amber: active ? "border-amber-300 bg-amber-50" : "border-slate-200 bg-white hover:border-amber-200",
    emerald: active ? "border-emerald-300 bg-emerald-50" : "border-slate-200 bg-white hover:border-emerald-200",
    red: active ? "border-red-300 bg-red-50" : "border-slate-200 bg-white hover:border-red-200",
    slate: active ? "border-slate-400 bg-slate-50" : "border-slate-200 bg-white hover:border-slate-300"
  }[tone];
