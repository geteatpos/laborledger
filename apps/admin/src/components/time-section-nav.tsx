...
import { isNavHrefActive, timeSectionNavItems } from "../lib/admin-nav-config";

export function TimeSectionNav() {
  const pathname = usePathname();

  return (
    <nav
      aria-label="Time sections"
      className="mb-6 flex flex-wrap gap-1 rounded-xl border border-slate-200/80 bg-white p-1 shadow-sm shadow-slate-200/20"
    >
      {timeSectionNavItems.map((item) => {
