...
import { PunchTimelineInline } from "./punch-timeline-inline";
import { TIMECARDS_REVIEW_HELPER } from "../lib/time-module-copy";
import {
  addDaysToDateKey,
  enrichShiftViews,
  filterShiftsByQuery,
...
export function SchedulingWorkspace({
  ...
  const [expandedPunchShiftId, setExpandedPunchShiftId] = useState<string | null>(null);
  ...
  return (
    <>
      <p className="mb-4 rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm text-slate-600">
        {TIMECARDS_REVIEW_HELPER}
      </p>
      ...
      {filteredShifts.length === 0 ? (
        <EmptyState
          title={shifts.length === 0 ? "No shifts scheduled yet" : "No shifts match your filters"}
          description={
            shifts.length === 0
                  ...
                  <table className="min-w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {group.shifts.map((shift) => {
                        const isPunchExpanded = expandedPunchShiftId === shift.id;
                        return (
                          <>
                            <tr key={shift.id} className="transition hover:bg-slate-50/80">
                              ...
                              <td className="px-5 py-3.5">
                                <button
                                  type="button"
                                  onClick={() => {
                                    setExpandedPunchShiftId(isPunchExpanded ? null : shift.id);
                                  }}
                                ...
                                >
                                  {shift.punchEvents && shift.punchEvents.length > 0
                                    ? `${shift.punchEvents.length} event${shift.punchEvents.length !== 1 ? "s" : ""}`
                                    : "View"}
                                </button>
                              </td>
                              {canManageCompany ? (
                            ...
                            {isPunchExpanded ? (
                              <tr key={`${shift.id}-punches`} className="bg-slate-50/40">
                                <td colSpan={canManageCompany ? 7 : 6} className="px-5 py-3">
                                  <PunchTimelineInline
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`mt-3 w-full rounded-md border px-2 py-1.5 text-xs font-medium transition ${
                          isPunchExpanded
                            ? "border-brand-300 bg-brand-50 text-brand-700"
                            : "border-slate-200 text-slate-700 hover:bg-slate-50"
                        }`}
                      >
                        {isPunchExpanded
                          ? "Hide punches"
                          : shift.punchEvents && shift.punchEvents.length > 0
                            ? `Punches (${shift.punchEvents.length})`
                            : "View punches"}
                      </button>
                      {isPunchExpanded ? (
                        <div className="mt-3">
