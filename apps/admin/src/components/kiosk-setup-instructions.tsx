...
export function KioskSetupInstructions({
  ...
  kioskSecret,
  apiUrl,
  title = "Clock device configuration"
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-950">
      ...
      <p className="mt-1 text-xs text-amber-800">
        Copy these values into your clock device deployment environment. {KIOSK_SECRET_HELPER}
      </p>
      ...
      <p className="mt-2 text-xs text-amber-800">
        After rotating a secret, update the clock device configuration before the old secret stops working.
      </p>
    </div>
  );
}
