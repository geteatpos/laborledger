  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-700" htmlFor={`edit-service-sort-${item.id}`}>
            Sort order
          </label>
          <input
            id={`edit-service-sort-${item.id}`}
            type="number"
            step={1}
            value={sortOrder}
            ...
            disabled={isSubmitting}
          />
          {fieldErrors.sortOrder ? <p className="mt-1 text-xs text-red-600">{fieldErrors.sortOrder}</p> : null}
        </div>
