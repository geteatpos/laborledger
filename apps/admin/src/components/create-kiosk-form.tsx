...
export function CreateKioskForm({ companyId, availableLocations, apiUrl }: CreateKioskFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setSubmitError(payload.message ?? "Unable to create clock device.");
      return;
    }
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {isOpen ? "Cancel" : "Add clock device"}
        </button>
        {noLocationsAvailable ? (
          <p className="text-sm text-slate-500">
            Every active location already has a clock device, or no active locations exist.
          </p>
        ) : null}
      </div>
      ...
      {createdKiosk ? (
        <KioskSetupInstructions
          ...
          apiUrl={apiUrl}
          title={`New clock device: ${createdKiosk.kiosk.name}`}
        />
      ) : null}
      ...
      {isOpen ? (
        ...
        >
          <h3 className="text-sm font-semibold text-slate-900">Add clock device</h3>
          <p className="mt-1 text-xs text-slate-500">
            Each location can have one clock device. A pairing secret is generated automatically.
          </p>
          ...
          >
            {isSubmitting ? "Creating…" : "Add clock device"}
          </button>
        </form>
      ) : null}
    </div>
