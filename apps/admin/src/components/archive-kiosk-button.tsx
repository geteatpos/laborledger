...
export function ArchiveKioskButton({
  ...
  async function handleConfirm() {
    ...
    if (!response.ok) {
      setErrorMessage(payload.message ?? "Unable to update clock device status.");
      return;
    }
  ...
  const confirmTitle = isArchived ? "Reactivate clock device?" : "Deactivate clock device?";
  const confirmBody = isArchived
    ? `${kioskName} can authenticate again after reactivation. Update the clock device if credentials changed while inactive.`
