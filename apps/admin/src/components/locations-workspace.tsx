  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-sm font-semibold text-slate-900">All locations</h2>
          <p className="mt-1 text-sm text-slate-500">
            {filteredLocations.length} {filteredLocations.length === 1 ? "location" : "locations"}
            {initialStatus !== "active" ? ` · ${initialStatus} filter` : ""}
          </p>
        </div>
        {canManageCompany ? (
      ...
      {filteredLocations.length === 0 ? (
        locations.length === 0 ? (
          <EmptyState
            ...
            description="Create a location to define where employees clock in and where supervisors have access."
            action={
              canManageCompany ? (
                <CreateLocationForm companyId={selectedCompany.id} serviceClients={serviceClients} />
              ) : undefined
