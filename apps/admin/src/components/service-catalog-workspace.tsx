  ...
  return (
    <>
      ...
      {filteredItems.length === 0 ? (
        <EmptyState
          title={items.length === 0 ? "No services yet" : "No services match your filters"}
          description={
            items.length === 0
              ? "Create services with fixed prices for reception and field work selection."
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredItems.map((item) => (
                    <tr key={item.id} className="transition hover:bg-slate-50/80">
                      ...
                      <td className="px-5 py-3.5 text-slate-600">{item.sortOrder ?? 0}</td>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Order {item.sortOrder ?? 0} · {item.category ?? "No category"}
                    </p>
