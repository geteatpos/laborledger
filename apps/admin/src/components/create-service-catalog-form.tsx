...
export function CreateServiceCatalogForm({ companyId }: CreateServiceCatalogFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sortOrderParsed = Number.parseInt(sortOrder.trim(), 10);
    const sortOrderError = !Number.isInteger(sortOrderParsed) ? "Sort order must be an integer." : undefined;

    if (nameError || priceError || sortOrderError) {
      setFieldErrors({
        ...(nameError ? { name: nameError } : {}),
        ...(priceError ? { priceDollars: priceError } : {}),
        ...(sortOrderError ? { sortOrder: sortOrderError } : {})
      });
      return;
    }
      ...
      {isOpen ? (
        ...
        >
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700" htmlFor="service-catalog-sort">
                Sort order
              </label>
              <input
                id="service-catalog-sort"
                type="number"
                step={1}
                value={sortOrder}
              ...
              {fieldErrors.sortOrder ? (
                <p className="mt-1.5 text-sm text-red-600">{fieldErrors.sortOrder}</p>
              ) : (
                <p className="mt-1.5 text-xs text-slate-500">Lower numbers appear first in the field checklist.</p>
              )}
            </div>
