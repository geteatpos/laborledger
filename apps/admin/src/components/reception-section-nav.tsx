...
import { isNavHrefActive, receptionSectionNavItems } from "../lib/admin-nav-config";

export function ReceptionSectionNav() {
  const pathname = usePathname();

  return (
    <nav
      aria-label="Reception sections"
      className="mb-6 flex flex-wrap gap-1 rounded-xl border border-slate-200/80 bg-white p-1 shadow-sm shadow-slate-200/20"
    >
      {receptionSectionNavItems.map((item) => {
