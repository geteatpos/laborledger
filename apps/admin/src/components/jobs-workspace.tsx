...
import {
  ...
  formatJobAssignedEmployee,
  formatJobCompletedAt,
  formatJobCreatedAt,
  formatJobInvoiceStatus,
  formatJobServicesSummary,
          ...
          action={
            filtersActive ? (
              <Link
                ...
                className="inline-flex rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
              >
                {JOBS_CLEAR_FILTERS_CTA}
              </Link>
            ) : canManageCompany ? (
                ...
                <tbody className="divide-y divide-slate-100">
                  {workOrders.map((workOrder) => (
                    <tr key={workOrder.id} className="transition hover:bg-slate-50/80">
                      ...
                      <td className="px-5 py-3.5 text-slate-600">
                        <span className="block font-medium text-slate-800">{workOrder.serviceClient.name}</span>
                        <span className="block text-xs text-slate-400">{workOrder.location.name}</span>
                      </td>
                      <td className="px-5 py-3.5 text-slate-600">{formatJobServicesSummary(workOrder)}</td>
                      <td className="px-5 py-3.5 text-slate-600">
                        {formatJobAssignedEmployee(workOrder)}
                      </td>
                      <td className="px-5 py-3.5 text-slate-600">
            ...
            {workOrders.map((workOrder) => (
              ...
              >
                ...
                <p className="mt-3 text-xs text-slate-500">
                  Customer: {workOrder.serviceClient.name} · {workOrder.location.name}
                </p>
                <p className="mt-2 text-sm text-slate-700">
                  {formatJobServicesSummary(workOrder)} · Assigned: {formatJobAssignedEmployee(workOrder)} ·{" "}
                  {formatJobInvoiceStatus(workOrder.status)}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  Received {formatJobCreatedAt(workOrder.createdAt)}
