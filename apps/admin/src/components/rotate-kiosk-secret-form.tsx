...
export function RotateKioskSecretForm({ kiosk, apiUrl }: RotateKioskSecretFormProps) {
  ...
  async function handleRotate() {
    ...
    if (!response.ok) {
      setSubmitError(payload.message ?? "Unable to rotate device secret.");
      return;
    }
  ...
  return (
    <div className="space-y-3">
      ...
      {isArchived ? (
        <p className="text-xs text-slate-500">Reactivate this clock device before rotating its secret.</p>
      ) : null}
      ...
      {isConfirmOpen ? (
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4" role="dialog">
          <h4 className="text-sm font-semibold text-slate-900">Rotate device secret?</h4>
          <p className="mt-1 text-sm text-slate-600">
            The current secret stops working immediately. Update the clock device configuration with the
