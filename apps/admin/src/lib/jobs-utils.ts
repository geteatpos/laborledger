...
export function formatJobServicesSummary(
  workOrder: Pick<WorkOrderListRecord, "serviceLines" | "serviceLineCount">
) {
  const names = workOrder.serviceLines.map((line) => line.serviceNameSnapshot);

  if (names.length === 0) {
    return workOrder.serviceLineCount === 1 ? "1 service" : `${workOrder.serviceLineCount} services`;
  }
...
export function formatJobAssignedEmployee(
  workOrder: Pick<WorkOrderListRecord, "assignedEmployee">
) {
  return workOrder.assignedEmployee?.fullName ?? "Unassigned";
}
