...
export function formatServiceLineCompletionStatus(
  ...
  if (workOrderStatus === "COMPLETED" || workOrderStatus === "INVOICED") {
    return "No realizado";
  }
...
export function formatServiceLineCompletionSummary(
  ...
  if (!completion) {
    if (workOrderStatus === "COMPLETED" || workOrderStatus === "INVOICED") {
      return "No realizado — not marked complete before finalize";
    }
...
export function assignmentDisclaimer() {
  return "Assignments identify who is responsible for a vehicle/work order. They do not replace clock punches and are not payroll.";
}
...
export function formatWorkOrderTotalDuration(totalDurationMs?: number | null) {
