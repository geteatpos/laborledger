...
export const CORRECTIONS_DISCLAIMER =
  "Corrections do not erase original clock punch events. Approved corrections store final values separately with actor, timestamp, and reason.";

export function formatTimelineEventSource(source: "kiosk" | "correction" | string): string {
  if (source === "kiosk") {
    return "Clock device";
  }
