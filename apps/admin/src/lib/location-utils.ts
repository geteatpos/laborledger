...
export type LocationRecord = {
  ...
  createdAt: string;
  updatedAt?: string;
  /** Service clients that can use this location (primary + shared links). */
  linkedServiceClientIds?: string[];
};
...
export function isLocationUsableByClient(
  location: Pick<LocationRecord, "serviceClientId" | "linkedServiceClientIds" | "archivedAt">,
  serviceClientId: string,
  options?: { includeArchived?: boolean }
) {
  if (!options?.includeArchived && location.archivedAt) {
    return false;
  }
  ...
  return (location.linkedServiceClientIds ?? []).includes(serviceClientId);
}
