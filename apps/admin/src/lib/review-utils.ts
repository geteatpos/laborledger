...
export function summarizeReviewShiftCounts(shifts: ReviewShiftRecord[]) {
  let needsReview = 0;
  let approved = 0;
  let incomplete = 0;
  let exceptions = 0;
  ...
  for (const shift of shifts) {
    if (shift.displayStatus === "needs_review") {
      needsReview += 1;
    } else if (shift.displayStatus === "approved") {
      approved += 1;
    } else if (shift.displayStatus === "incomplete") {
      incomplete += 1;
    }
  ...
  return { needsReview, approved, incomplete, exceptions };
}
