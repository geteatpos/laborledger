...
import { JOBS_MODULE_DESCRIPTION, JOBS_WORKFLOW_HELPER } from "../../../lib/operations-module-copy";
      ...
      <AdminShell
        ...
        actions={
          <span className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm font-medium text-slate-700">
            {workOrders.length} {workOrders.length === 1 ? "job" : "jobs"}
          </span>
        }
      >
        <JobsSectionNav />
        <EmployeesModuleIntro help={JOBS_WORKFLOW_HELPER}>{JOBS_MODULE_DESCRIPTION}</EmployeesModuleIntro>
        <JobsWorkspace
