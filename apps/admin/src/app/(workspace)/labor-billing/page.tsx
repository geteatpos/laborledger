...
import {
  LABOR_BILLING_MODULE_DESCRIPTION,
  LABOR_BILLING_MODULE_TITLE
...
export default async function LaborBillingPage({ searchParams }: LaborBillingPageProps) {
  try {
    ...
    if (workspace.blocked) {
      return (
        <AdminShell
          title={LABOR_BILLING_MODULE_TITLE}
          description={LABOR_BILLING_MODULE_DESCRIPTION}
        >
    ...
    const drafts = await apiGet<LaborBillingDraftSummary[]>(
      `/company-operations/companies/${selectedCompany.id}/labor-pay-billing/drafts`,
      cookieHeader
    );

    let selectedDraft: LaborBillingDraftDetail | null = null;
    if (query.draftId) {
      selectedDraft = await apiGet<LaborBillingDraftDetail>(
        `/company-operations/companies/${selectedCompany.id}/labor-pay-billing/drafts/${query.draftId}`,
        cookieHeader
      );
    }
    ...
    return (
      ...
      >
        <BillingSectionNav />
        <BillingModuleIntro>{LABOR_BILLING_MODULE_DESCRIPTION}</BillingModuleIntro>
