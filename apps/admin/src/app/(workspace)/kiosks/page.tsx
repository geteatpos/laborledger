...
import {
  LIVE_CLOCK_MODULE_DESCRIPTION,
  LIVE_CLOCK_MODULE_TITLE,
  LIVE_CLOCK_HELPER
...
export default async function KiosksPage({ searchParams }: KiosksPageProps) {
  try {
    ...
    if (workspace.blocked) {
      return (
        <AdminShell title={LIVE_CLOCK_MODULE_TITLE} description={LIVE_CLOCK_MODULE_DESCRIPTION}>
          <TimeSectionNav />
    ...
    return (
      <AdminShell
        ...
        description={LIVE_CLOCK_MODULE_DESCRIPTION}
        actions={
          <span className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm font-medium text-slate-700">
            {visibleKiosks.length} {visibleKiosks.length === 1 ? "clock device" : "clock devices"}
          </span>
        }
      >
        <TimeSectionNav />
        <EmployeesModuleIntro help={LIVE_CLOCK_HELPER}>{LIVE_CLOCK_MODULE_DESCRIPTION}</EmployeesModuleIntro>
        <KiosksWorkspace
          companies={companies}
