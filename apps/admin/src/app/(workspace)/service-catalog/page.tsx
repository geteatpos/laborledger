...
import {
  ...
  SERVICES_NOT_COMPLETED_WORK_COPY
...
export default async function ServiceCatalogPage({ searchParams }: ServiceCatalogPageProps) {
  try {
    ...
    if (workspace.blocked) {
      return (
        <AdminShell title={SERVICES_MODULE_TITLE} description={SERVICES_MODULE_DESCRIPTION}>
          <JobsSectionNav />
    ...
    return (
      ...
      >
        <JobsSectionNav />
        <EmployeesModuleIntro help={SERVICES_NOT_COMPLETED_WORK_COPY}>
          {SERVICES_MODULE_DESCRIPTION}
