...
import {
  TIMECARDS_MODULE_DESCRIPTION,
  TIMECARDS_MODULE_TITLE,
...
export default async function SchedulingPage({ searchParams }: SchedulingPageProps) {
  try {
    ...
    if (workspace.blocked) {
      return (
        <AdminShell title={TIMECARDS_MODULE_TITLE} description={TIMECARDS_MODULE_DESCRIPTION}>
          <TimeSectionNav />
    ...
    return (
      <AdminShell
        title={TIMECARDS_MODULE_TITLE}
        description={TIMECARDS_MODULE_DESCRIPTION}
        actions={
      ...
      >
        <TimeSectionNav />
        <EmployeesModuleIntro help={TIMECARDS_REVIEW_HELPER}>
          {TIMECARDS_MODULE_DESCRIPTION}
