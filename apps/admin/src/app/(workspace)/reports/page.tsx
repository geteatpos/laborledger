...
import {
  REPORTS_MODULE_DESCRIPTION,
  REPORTS_MODULE_TITLE
} from "../../../lib/billing-module-copy";
import type { FinancialSummaryResponse } from "../../../lib/financial-dashboard-utils";
import {
...
export default async function ReportsPage({ searchParams }: ReportsPageProps) {
  try {
    ...
    if (workspace.blocked) {
      return (
        <AdminShell
          ...
          description={REPORTS_MODULE_DESCRIPTION}
        >
    ...
    const financialSummary = await apiGet<FinancialSummaryResponse>(
      `/company-operations/companies/${selectedCompany.id}/financial-summary?from=${encodeURIComponent(range.from)}&to=${encodeURIComponent(range.to)}&preset=custom`,
      cookieHeader
    );
    ...
    return (
      ...
      >
        <BillingModuleIntro>{REPORTS_MODULE_DESCRIPTION}</BillingModuleIntro>
        <div className="mb-8">
          <FinancialReportsSummary summary={financialSummary} />
        </div>
