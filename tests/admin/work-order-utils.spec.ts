...
describe("work-order-utils", () => {
  ...
  it("uses assignment disclaimer that excludes payroll and clock punch replacement", () => {
    const copy = assignmentDisclaimer();
    expect(copy.toLowerCase()).toContain("not payroll");
    expect(copy.toLowerCase()).toContain("clock punches");
  ...
  it("formats service line completion status and progress", () => {
    ...
    expect(formatServiceLineCompletionStatus(null, "COMPLETED")).toBe("No realizado");
    expect(formatServiceLineCompletionStatus(null, "INVOICED")).toBe("No realizado");
    ...
    expect(formatServiceLineCompletionSummary(null, "COMPLETED")).toContain("No realizado");
    expect(
      formatWorkOrderCompletionProgress({
