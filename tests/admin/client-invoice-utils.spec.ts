...
describe("client-invoice-utils", () => {
  ...
  it("uses disclaimer that distinguishes labor billing from customer invoices", () => {
    const copy = clientInvoiceDisclaimer();
    expect(copy.toLowerCase()).toContain("labor billing");
    expect(copy.toLowerCase()).toContain("approved clock time");
    expect(copy.toLowerCase()).toContain("invoices are customer-facing");
    expect(copy.toLowerCase()).toContain("completed service work");
  });
