...
const DEMO_FIELD_HOSTNAME = "localhost";
const DEMO_FIELD_SERVICE_SECRET = "demo-field-service-secret-dev";
...
async function ensureFieldSite(
  tx: Prisma.TransactionClient,
  input: {
    groupId: string;
    companyId: string;
  ...
  const hostname = input.hostname.trim().toLowerCase().split(":")[0] ?? input.hostname.trim().toLowerCase();

  return tx.fieldSite.upsert({
...
async function main() {
  ...
  try {
    ...
    const result = await prisma.$transaction(
      ...
      const fieldSite = await ensureFieldSite(tx, {
        groupId: group.id,
        companyId: company.id,
        locationId: primaryLocation.id,
        hostname: DEMO_FIELD_HOSTNAME,
    ...
    console.log(`Field hostname (dev only): ${result.fieldSite.hostname}`);
    console.log(`Field service secret (dev only): ${DEMO_FIELD_SERVICE_SECRET}`);
    console.log(
    ...
    console.log(
      "Field local dev: set FIELD_SERVICE_SECRET in apps/api and apps/field, then open http://localhost:3001/field/login (bootstrap uses hostname localhost)."
    );
    console.log("Legacy env fallback (optional): WORKER_COMPANY_ID, KIOSK_ID, KIOSK_SECRET in apps/field/.env.local.");
